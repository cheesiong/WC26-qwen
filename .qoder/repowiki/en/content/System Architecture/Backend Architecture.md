...
- Scheduling: node-cron jobs for live result sync and batch prediction regeneration.
...
subgraph "Scheduling"
Cron["node-cron Jobs<br/>live sync + prediction cron"]
end
Express --> Routes
Routes --> Pred
Routes --> Data
...
- Scheduling via node-cron for live result sync and batch prediction regeneration.
...
- Application: Services encapsulate business logic for predictions, data ingestion, analysis, and bracket progression.
...
**Diagram sources**
- [server.js:21-681](file://backend/server.js#L21-L681)
...
BackBone --> Signals["Fetch signals:<br/>form, intel, H2H, lineup, rest"]
...
PredBatch --> Notify["Notify IndexNow"]
Notify --> CronEnd["Done"]
```

**Diagram sources**
- [server.js:585-633](file://backend/server.js#L585-L633)
